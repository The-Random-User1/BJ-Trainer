...
static const int32_t SESSION_TIME=600L; //session time (for modes that end on a timer)
...
static const uint32_t STOPWATCH_MODE_TOTAL_ENGAGE_TIME_TARGET=400L; //the total amount of time in seconds you intend on needing to engage.
static const uint32_t COUNTING_MODE_TOTAL_STROKES_TARGET=100L;      //the total number of strokes to complete.
...
void bt_lcd_printf(int row,const char* fmt,...)
{
  char outbuf[18];
  va_list argptr;
    va_start(argptr, fmt);
